	...
	__approvedUpdated : function(xml) {
		...
		this.approved[0] = "Rafael";
			...
			this.feed(result).find('item').each(function(){
				...
				//confirmation = title.slice(0, 27);
	
				//j = 0;
				//if(confirmation == "janelasvirtuais: Aprovado: "){
					//this.approved[j] = title.slice(27);
	...
	__feedUpdated : function(result) {
      ...
      var img = imgElement.getAttribute("url");
			...
			//if (result.feed.entries[i]) {
				...
				this.tweetQueue.push( '<img align="top" src="'+img+'" /><span class="tweetauthor2">(' + author.replace(/ \(.*$/,'') + ')</span> ' + title + this.approved[0]);
			//}
		//}
		}
