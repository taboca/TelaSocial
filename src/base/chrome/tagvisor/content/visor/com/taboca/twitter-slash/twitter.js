 ...
 name   : __appName,
	...
	start : function() {
		...
		this.element.className="twitterPanel";
		 this.element.setAttribute("style","width:100%;height:100%;padding-top:1em;");
	...
	__feedUpdated : function(result) {
		...
		for (i = 0; i < result.feed.entries.length; i++) {
			if (result.feed.entries[i]) {
				this.tweetQueue.push( '<img src="http://go.bath.ac.uk/qr/download?DATA='+result.feed.entries[i].link+'" style="margin-right:15px; margin-bottom:10px; " align="left" />'+result.feed.entries[i].content + ' <span class="tweetauthor">(' + result.feed.entries[i].author.replace(/ \(.*$/,'') + ')</span>');
			}
		}
