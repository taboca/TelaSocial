	...
	readStep: function () { 
		...
		for(var i=0;i<this.readIndex;i++) { 
			...
			if(i==0) { 
				...
				this._coreDoc.getElementById("typingexpanded").innerHTML= "<span class='padding:1em;font-size:20px'>"+ elCurr.description + "</span>";
				var jq = this._service_jquery;

				jq("#typingexpanded").find('img').each(function(){
                       			 var img = jq(this).attr("src");
					 var imgfull = "http://softwarelivre.org/" + img;
					 jq(this).attr("src",imgfull);
